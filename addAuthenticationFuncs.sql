...
create view users_view as select users.name,users.division,users.department,
users.username,users.password 
from users;
create or replace rule ins_user as on insert to users_view
do instead insert into users(name,division,department,username,enc_pass,salt)
values (new.name,new.division,new.department,new.username,
